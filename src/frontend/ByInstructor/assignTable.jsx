import {
    Tooltip, Table, TableHeader, Modal, ModalContent, Kbd, Button, ModalFooter, Pagination, ModalHeader, ModalBody, useDisclosure, TableColumn, TableBody, TableRow, TableCell,User
...
export default function PositionTable() {
    ...
    useEffect(() => {
        const getAssign = async () => {
            await apiInstance.get(`assignments`, { params: { limit: 80, rowsPerPage: rowsPerPage } })
                .then(res => {
                  console.log(res.data.data,'res')
const obj=(res.data.data.map((i)=>(
    JSON.parse(i.links)
)))
console.log(obj,'res')
    ...
    const handleDelete = async () => {
        ...
        await apiInstance.delete('assignments/' + delID)
            .then(() => {
    ...
    return (
        <div className='mx-8 mb-10'>
            ...
            <div className="flex justify-between items-center mb-3">
                <span className="text-default-400 text-small">Total {assignList.length} Assignments</span>
            ...
            >
               ...
               /></TableCell>
                              <TableCell>{item?.subject?.title}</TableCell>
                               <TableCell>{JSON.parse(item.links
).map((e)=>(
    <div key={e} className='text-blue-700 gap-5'>
   <a target='_blank' href={e.links}>{e.links}</a>
</div>
))}</TableCell>
