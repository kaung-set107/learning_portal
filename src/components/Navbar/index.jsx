...
export default function NavBar() {
  ...
  const resetPassword = async () => {
    ...
    await apiInstance.put('users/reset', payload)
      .then((res) => {
        Swal.fire({
          icon: 'success',
          title: 'Success!',
  ...
  const getSetting = async () => {
    await apiInstance.get('setting/' + '651a47a7e259234bf081204c') //main setting id
      .then(res => {
          ...
          >
            ...
            <Dropdown placement='bottom-end mt-1'>
              ...
              <DropdownMenu aria-label='Profile Actions' variant='flat'>
                ...
                <DropdownItem key='system' color='primary' onClick={handleSettings}>System Settings</DropdownItem>
                  ...
                  <ModalBody>
                    ...
                    <Card
                      ...
                      <CardBody>
                        {console.log(setting)}
                        <div className='flex flex-col gap-2'>
                          <label className='font-semibold text-medium'>First Penalty</label>
                          <div className='flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-0 gap-4'>
                            <Input
                              type='text'
                              placeholder='Time'
                              defaultValue={setting?.fstpenalty?.pTime}
                              onChange={(e) => handleFirstPenalty('pTime', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Percent'
                              defaultValue={setting?.fstpenalty?.pPercent}
                              onChange={(e) => handleFirstPenalty('pPercent', e.target.value)}// Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Amount'
                              defaultValue={setting?.fstpenalty?.pAmount} // Disable scrolling
                              onChange={(e) => handleFirstPenalty('pAmount', e.target.value)}
                            />
                          </div>

                          <label className='font-semibold text-medium'>Second Penalty</label>
                          <div className='flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-0 gap-4'>
                            <Input
                              type='text'
                              placeholder='Time'
                              defaultValue={setting?.secpenalty?.pTime}
                              onChange={(e) => handleSecPenalty('pTime', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Percent'
                              defaultValue={setting?.secpenalty?.pPercent}
                              onChange={(e) => handleSecPenalty('pPercent', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Amount'
                              defaultValue={setting?.secpenalty?.pAmount} // Disable scrolling
                              onChange={(e) => handleSecPenalty('pAmount', e.target.value)}
                            />
                          </div>

                          <label className='font-semibold text-medium'>Third Penalty</label>
                          <div className='flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-0 gap-4'>
                            <Input
                              type='text'
                              placeholder='Time'
                              defaultValue={setting?.thdpenalty?.pTime}
                              onChange={(e) => handleThirdPenalty('pTime', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Percent'
                              defaultValue={setting?.thdpenalty?.pPercent}
                              onChange={(e) => handleThirdPenalty('pPercent', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Amount'
                              defaultValue={setting?.thdpenalty?.pAmount}// Disable scrolling
                              onChange={(e) => handleThirdPenalty('pAmount', e.target.value)}
                            />
                          </div>

                          <label className='font-semibold text-medium'>Final Penalty</label>
                          <div className='flex w-full flex-wrap md:flex-nowrap mb-6 md:mb-0 gap-4'>
                            <Input
                              type='text'
                              placeholder='Time'
                              defaultValue={setting?.fnlpenalty?.pTime}
                              onChange={(e) => handleFinalPenalty('pTime', e.target.value)} // Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Percent'
                              defaultValue={setting?.fnlpenalty?.pPercent}
                              onChange={(e) => handleFinalPenalty('pPercent', e.target.value)}// Disable scrolling
                            />
                            <Input
                              type='number'
                              placeholder='Amount'
                              defaultValue={setting?.fnlpenalty?.pAmount} // Disable scrolling
                              onChange={(e) => handleFinalPenalty('pAmount', e.target.value)}
                            />
                          </div>
                        </div>
